...
function FakeCover:init()
    ...
    if is_pathchooser == false then
        ...
        titlefont = burrow_util.title_serif
        ...
        if not title then -- use filename as title (big and centered)
            titlefont = burrow_util.good_serif
            bold_title = true
            title = filename
        ...
        authors = burrow_util.formatAuthors(self.authors, 3)
        ...
        while true do
            ...
            if authors then
                authors_wg = TextBoxWidget:new {
                    text = authors,
                    lang = self.book_lang,
                    face = Font:getFace(burrow_util.good_serif, math.max(self.authors_font_max - sizedec, self.authors_font_min)),
                    width = text_width,
                    alignment = "center",
            ...
            if series then
                series_wg = TextBoxWidget:new {
                    text = series,
                    lang = self.book_lang,
                    face = Font:getFace(burrow_util.good_serif, math.max(self.series_font_max - sizedec, self.series_font_min)),
                    width = text_width,
                    alignment = "center",
                    bgcolor = self.background,
                }
...
function MosaicMenuItem:update()
    ...
    if self.is_directory then
        ...
        if is_pathchooser == false then
            ...
            subfolder_cover_image = burrow_util.getFolderCover(self.filepath, dimen.w, dimen.h, BurrowMigration.folderCoverPath(self.entry))
            ...
            local function build_directory_text(font_size, height, baseline)
                directory_text = TextWidget:new {
                    ...
                    face = Font:getFace(burrow_util.good_serif, font_size),
                    max_width = dimen.w,
                    alignment = "center",
                    padding = 0,
            ...
            local dirtext_font_size = burrow_util.grid_defaults.dir_font_nominal
            ...
            while dirtext_font_size > burrow_util.grid_defaults.dir_font_min do
                if directory_text:isTruncated() then
                    dirtext_font_size = math.min(dirtext_font_size - burrow_util.grid_defaults.fontsize_dec_step, burrow_util.grid_defaults.dir_font_min)
        ...
        if bookinfo and is_pathchooser == false then -- This book is known
            ...
            self.status = status
            self.pages, self.show_progress_bar = burrow_util.showProgressBar(bookinfo.pages)
            ...
            if self.do_cover_image and bookinfo.has_cover and not bookinfo.ignore_cover then
                ...
                local img_width
                local img_height
                if burrow_util.grid_defaults.stretch_covers then
                    scale_factor = nil
                    img_width = (max_img_w * burrow_util.grid_defaults.stretch_ratio) - border_total
                ...
                if bookinfo.series and bookinfo.series_index then
                    ...
                    bookinfo.series = "#" .. bookinfo.series_index .. burrow_util.separator.en_dash .. BD.auto(bookinfo.series)
                end
...
function MosaicMenuItem:paintTo(bb, x, y)
    ...
    if x ~= math.floor(x) or y ~= math.floor(y) then
        logger.err(burrow_debug.logprefix, "MosaicMenuItem:paintTo() got non-integer x/y :", x, y)
    end
    ...
    if bookinfo and self.init_done then
        ...
        if series_mode == "series_in_separate_line" and show_series and is_pathchooser == false then
            local series_index = burrow_util.formatSeriesIndex(bookinfo.series_index)
            if series_index ~= nil then
                ...
                local series_widget_text = TextWidget:new {
                    text = series_index,
                    face = Font:getFace(burrow_util.good_serif, 14),
                    alignment = "left",
                    padding = 0,
                }
        ...
        if self.show_progress_bar and is_pathchooser == false then
            ...
            if est_page_count then
                ...
                local max_progress_size = burrow_util.grid_defaults.progress_bar_max_size
                local pages_per_pixel = burrow_util.grid_defaults.progress_bar_pages_per_pixel
                local min_progress_size = burrow_util.grid_defaults.progress_bar_min_size
        ...
        elseif is_pathchooser == false then
            ...
            if progresstxt ~= nil then
                local txtprogress_widget_text = TextWidget:new {
                    text = progresstxt,
                    face = Font:getFace(burrow_util.good_sans, 15),
                    alignment = "center",
                    padding = Size.padding.tiny,
                }
...
function MosaicMenu:_updateItemsBuildUI()
    -- Build our grid
    local grid_timer = burrow_debug:new()
    ...
    for idx = 1, self.perpage do
        ...
        if idx % self.nb_cols == 1 then -- new row
            ...
            local baseline = is_last_row and burrow_util.thinWhiteLine or burrow_util.thinGrayLine
                ...
                elseif row_start <= self.recent_boundary_index and row_end >= self.recent_boundary_index then
                    ...
                    table.insert(self.item_group, OverlapGroup:new {
                        ...
                        LeftContainer:new {
                            ...
                            burrow_util.thinBlackLine((2 * pad) + dark_inner),
                        },
                    })
                else
