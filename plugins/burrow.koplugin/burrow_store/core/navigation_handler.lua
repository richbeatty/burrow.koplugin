...
local CatalogUtils = require("burrow_store.utils.catalog_utils")
local BrowserContext = require("burrow_store.core.browser_context")
			...
			if link.type ~= nil then
				...
				if not browser_context.sync then
					...
					if link.type:find(browser_context.search_type) then
						if link.href then
							local FeedFetcher = require("burrow_store.core.feed_fetcher")
		...
		if entry.link then
			for ___, link in ipairs(entry.link) do
				...
				if link.rel or link.title then
					-- Cover and thumbnail links must be classified before document
					-- acquisitions. KOReader can open JPEG and PNG files, but an OPDS
					-- cover is artwork, not an alternate book format.
					if browser_context.thumbnail_rel[link.rel] then
						item.thumbnail = link_href
					elseif browser_context.image_rel[link.rel] then
						item.image = link_href
					-- Borrow links
					elseif link.rel == browser_context.borrow_rel then
					...
					elseif link.rel == browser_context.stream_rel then
						...
						-- Acquisition links (downloads). Image resources are ignored even
						-- when a catalog labels them as acquisitions.
					elseif UrlUtils.isAcquisitionLink(link, browser_context.acquisition_rel)
						and not DownloadManager.isImageLink(link) then
						table.insert(item.acquisitions, {
							type  = link.type,
							href  = link_href,
							title = link.title,
						})
						-- Other downloadable document types
					elseif link.rel ~= "alternate"
						and not DownloadManager.isImageLink(link)
						and DocumentRegistry:hasProvider(nil, link.type) then
						table.insert(item.acquisitions, {
							type  = link.type,
							href  = link_href,
							title = link.title,
...
function NavigationHandler.updateCatalog(item_url, browser, paths_updated)
	...
	if #menu_table > 0 or browser.facet_groups or browser.search_url then
		...
		if browser.facet_groups or browser.search_url then
			...
			-- Always expose the catalog menu. Even text-only catalogs need access
			-- to the download queue and the add-catalog action.
			browser.title_bar_left_icon = Constants.ICONS.MENU
			browser.onLeftButtonTap = function()
