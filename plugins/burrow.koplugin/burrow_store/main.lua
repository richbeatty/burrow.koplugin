...
local OPDSBrowser = require("burrow_store.ui.browser")
...
local Settings = require("burrow_store.config.settings")
local SettingsMenu = require("burrow_store.config.settings_menu")

-- Import settings dialogs
local SettingsDialogs = require("burrow_store.ui.dialogs.settings_dialogs")
...
local OPDS = WidgetContainer:extend {
    ...
    embedded = false,
    brand_title = nil,
    opds_settings_file = BurrowMigration.storeSettingsPath(),
    settings = nil,
    servers = nil,
    downloads = nil,
}
...
function OPDS:onDispatcherRegisterActions()
    Dispatcher:registerAction("burrow_store_show_catalog",
        { category = "none", event = "ShowOPDSCatalog", title = _("Burrow Store"), filemanager = true, }
    )

    Dispatcher:registerAction("burrow_store_sync_all",
        { category = "none", event = "StartOPDSSyncAllCatalogs", title = _("Burrow Store: Sync all catalogs"), filemanager = true, }
    )

    Dispatcher:registerAction("burrow_store_force_sync_all",
        { category = "none", event = "StartOPDSForceSyncAllCatalogs", title = _("Burrow Store: Force sync all catalogs"), filemanager = true, }
    )
end
...
function OPDS:_createBrowserInstance()
    return OPDSBrowser:new {
        ...
        title = self.brand_title or _("Burrow Store"),
        is_popout = false,
        is_borderless = true,
...
function OPDS:addToMainMenu(menu_items)
    if not self.embedded and not self.ui.document then -- FileManager menu only
        menu_items.burrow_store = {
            text = _("Burrow Store"),
