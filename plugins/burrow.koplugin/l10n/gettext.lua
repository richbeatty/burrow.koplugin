...
local changeLang = function(new_lang)
    ...
    if ok then
        ...
        logger.info(burrow_debug.logprefix, string.format("Failed to parse the [PO|MO] file for lang %s: %s", tostring(new_lang), tostring(err)))
    end
...
local function createGetTextProxy(new_gettext, gettext)
    if not (new_gettext.wrapUntranslated and new_gettext.translation and new_gettext.current_lang) then
        logger.dbg(burrow_debug.logprefix, string.format("NewGetText was not loaded correctly for lang %s", tostring(gettext.current_lang)))
        return gettext
    end
    ...
    return setmetatable({
        ...
        debug_dump = function()
            ...
            logger.info(burrow_debug.logprefix, string.format("debug_dump: %s.po to %s", new_lang, dump_path))
      end
    }, mt)
end
