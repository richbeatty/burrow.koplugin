...
local BOOKINFO_DB_SCHEMA = [[
    ...
    -- To keep track of Burrow settings
...
function BookInfoManager:init()
    self.db_location = BurrowMigration.libraryDatabasePath()
...
function BookInfoManager:createDB()
    ...
    --     logger.warn(burrow_debug.logprefix, "BookInfo cache DB schema updated from version", db_version, "to version", BOOKINFO_DB_VERSION)
    --     logger.warn(burrow_debug.logprefix, "Deleting existing", self.db_location, "to recreate it")
...
local function loadKoboMetadata(filepath, get_cover)
    ...
    local function getKoboVirtualLibrary()
        ...
        logger.dbg(burrow_debug.logprefix, "Kobo plugin instance:", kobo_plugin ~= nil)
    ...
    if isKoboVirtualPath(filepath) then
        logger.info(burrow_debug.logprefix, "Path is a Kobo virtual path, trying to get metadata from Kobo virtual library")
        local virtual_library = getKoboVirtualLibrary()
        logger.dbg(burrow_debug.logprefix, "Got Kobo virtual library:", virtual_library ~= nil)
...
function BookInfoManager:getBookInfo(filepath, get_cover)
    ...
    -- Burrow may be used by PathChooser, which will not filter out
...
function BookInfoManager:extractBookInfo(filepath, cover_specs)
    ...
    if prev_tries < self.max_extract_tries then
        if prev_tries > 0 then
            logger.dbg(burrow_debug.logprefix, "Seen", prev_tries, "previous attempts at info extraction", filepath, ", trying again")
        end
        dbrow.in_progress = prev_tries + 1 -- extraction not yet successful
    else
        logger.info(burrow_debug.logprefix, "Seen", prev_tries, "previous attempts at info extraction", filepath, ", too many, ignoring it.")
        tried_enough = true
            ...
            if not document:loadDocument(false) then -- load only metadata
                ...
                local function getEstimatedPagecount(fname)
                    ...
                    if fn_pagecount and fn_pagecount ~= "0" then
                        logger.dbg(burrow_debug.logprefix, "Pagecount found in filename", fname, fn_pagecount)
                        return fn_pagecount
                    end

                    if filetype ~= "epub" then
                        logger.dbg(burrow_debug.logprefix, "Skipping pagecount, not epub", fname)
                        return nil
                    end
                    ...
                    if Device:isAndroid() then
                        ...
                        logger.dbg(burrow_debug.logprefix, locate_opf_command)
                        ...
                        while true and fh ~= nil do
                            ...
                            logger.dbg(burrow_debug.logprefix, line)
                        end
                    ...
                    if opf_file then
                        ...
                        if found_value and found_value ~= "0" then
                            logger.dbg(burrow_debug.logprefix, "Pagecount found in opf metadata ", fname, found_value)
                            return found_value
                        end
                    end
                    logger.dbg(burrow_debug.logprefix, "Pagecount not found", fname)
                    return nil
                end
        ...
        if loaded then
            ...
            if cover_specs then
                ...
                if cover_bb then
                    ...
                    logger.dbg(burrow_debug.logprefix, "cover for", filename, "scaled from", dbrow.cover_sizetag, "to", cover_bb.w, "x",
    ...
    if #self.subprocesses_pids > 0 then
        ...
        if #self.subprocesses_pids > 0 then
            ...
            if time.now() > self.subprocesses_last_added_time + self.subprocesses_killall_timeout_time then
                logger.warn(burrow_debug.logprefix, "Some subprocesses were running for too long, killing them")
...
function BookInfoManager:terminateBackgroundJobs()
    logger.dbg(burrow_debug.logprefix, "terminating", #self.subprocesses_pids, "subprocesses")
...
function BookInfoManager:extractInBackground(files)
    ...
    local task = function()
        logger.dbg(burrow_debug.logprefix, "  BG extraction started")
        for idx = 1, #files do
            local filepath = files[idx].filepath
            local cover_specs = files[idx].cover_specs
            logger.dbg(burrow_debug.logprefix, "  BG extracting:", filepath)
            self:extractBookInfo(filepath, cover_specs)
        end
        logger.dbg(burrow_debug.logprefix, "  BG extraction done")
    end
    ...
    if not task_pid then
        logger.warn(burrow_debug.logprefix, "Failed lauching background extraction sub-process (fork failed)")
...
function BookInfoManager:cleanUp()
    ...
    if self.cleanup_needed then
        logger.dbg(burrow_debug.logprefix, "Removing directory", self.tmpcr3cache)
        ...
        for __, d in pairs(dirs) do
            ...
            for f in lfs.dir(d) do
                ...
                if recursive and attributes.mode == "directory" and f ~= "." and f ~= ".." and (G_reader_settings:isTrue("show_hidden") or not util.stringStartsWith(f, ".")) then
                    -- omit sidecar folders
                    if not util.stringEndsWith(f, ".sdr") then
