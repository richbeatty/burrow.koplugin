        ...
        self.items_update_action = function()
            ...
            while i <= #self.items_to_update do -- process and clean in-place
                ...
                if item.bookinfo_found then
                    logger.dbg(burrow_debug.logprefix, "  found", item.text)
            ...
            if #self.items_to_update > 0 then -- re-schedule myself
                if is_still_extracting then   -- we have still chances to get new stuff
                    ...
                    logger.dbg(burrow_debug.logprefix, "Not all items found, but background extraction has stopped, not re-scheduling")
                end
            else
                logger.dbg(burrow_debug.logprefix, "items update completed")
            end
        end
        UIManager:scheduleIn(1, self.items_update_action)
    end
    ...
    function file_chooser:showFileDialog(item)
        ...
        if is_file then
            ...
            if has_provider or been_opened then
                book_props = file_manager.burrow and file_manager.burrow:getBookInfo(file)
                if been_opened then
...
function CoverMenu:menuInit()
    ...
    local footer_font_face = burrow_util.good_serif
    local footer_font_size = burrow_util.footer_defaults.font_size
    if BookInfoManager:getSetting("replace_footer_text") then
        footer_font_face = burrow_util.good_sans
        footer_font_size = burrow_util.footer_defaults.font_size_deviceinfo
    end
    if not BookInfoManager:getSetting("reverse_footer") then
    ...
    local footer_line = BottomContainer:new {
        ...
        burrow_util.mediumBlackLine(self.inner_dimen.w),
    }
...
function CoverMenu:updatePageInfo(select_number)
    ...
    if not is_pathchooser and self.cur_folder_text and type(self.path) == "string" and self.path ~= '' then
        ...
        local footertxt = burrow_util.formatFooterText(self.footer_config, self._manager, self.path, filemanagerutil.getDefaultDir(),
