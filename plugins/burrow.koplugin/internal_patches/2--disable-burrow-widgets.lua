local MODULE_KEY = "burrow.internal.2__disable_burrow_widgets"
local existing_module = package.loaded[MODULE_KEY]
if existing_module then return existing_module end
local Module = { key = MODULE_KEY, phase = "instance", filename = "2--disable-burrow-widgets.lua" }
package.loaded[MODULE_KEY] = Module

--[[ 
User patch for Burrow - Disable specific UI elements
This patch disables:
...
local function patchDisableUIElements(plugin)
    ...
    if not ImageWidget.patched_disable_all_burrow_widgets then
        ImageWidget.patched_disable_all_burrow_widgets = true
    ...
    if MosaicMenuItem.patched_disable_all_burrow_widgets then
        return
    end
    MosaicMenuItem.patched_disable_all_burrow_widgets = true
...
Module.apply = patchDisableUIElements
return Module
