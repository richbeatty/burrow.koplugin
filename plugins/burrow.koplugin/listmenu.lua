...
local plugin_dir = burrow_util.getPluginDir()
...
function ListMenuItem:update()
    ...
    local fontsize_dec_step = burrow_util.list_defaults.fontsize_dec_step
    -- calculate font used in all right widget text
    local wright_font_size = _fontSize(burrow_util.list_defaults.wright_font_nominal, burrow_util.list_defaults.wright_font_max)
    local wright_font_face = Font:getFace(burrow_util.good_sans, wright_font_size)
    -- and font sizes used for title and author/series
    local title_font_size = _fontSize(burrow_util.list_defaults.title_font_nominal, burrow_util.list_defaults.title_font_max)
    local directory_font_size = _fontSize(burrow_util.list_defaults.directory_font_nominal, burrow_util.list_defaults.directory_font_max)
    local authors_font_size = _fontSize(burrow_util.list_defaults.authors_font_nominal, burrow_util.list_defaults.authors_font_max)
    ...
    is_pathchooser = burrow_util.isPathChooser(self)
    ...
    if self.is_directory then
        ...
        if is_pathchooser == false then
            ...
            wright_font_face = Font:getFace(burrow_util.good_sans, _fontSize(15, 19))
        ...
        if self.do_cover_image and is_pathchooser == false then
            ...
            subfolder_cover_image = burrow_util.getFolderCover(self.filepath, max_img_w * 0.82, max_img_h, BurrowMigration.folderCoverPath(self.entry))
            ...
            if subfolder_cover_image == nil and not BookInfoManager:getSetting("disable_auto_foldercovers") then
                subfolder_cover_image = burrow_util.getSubfolderCoverImages(self.filepath, max_img_w, max_img_h)
            end
        ...
        local folderfont = burrow_util.good_serif
        ...
        if is_pathchooser or self.do_filename_only then
            ...
            folderfont = burrow_util.good_sans
        end
        ...
        if bookinfo and is_pathchooser == false then -- This book is known (and not in patchooser mode)
            ...
            local wright_items = { align = "right" }
            local est_page_count, draw_progressbar = burrow_util.showProgressBar(bookinfo.pages)
            ...
            if draw_progressbar then
                ...
                local max_progress_size = burrow_util.list_defaults.progress_bar_max_size
                local pages_per_pixel = burrow_util.list_defaults.progress_bar_pages_per_pixel
                local min_progress_size = burrow_util.list_defaults.progress_bar_min_size
            ...
            if BookInfoManager:getSetting("hide_file_info") then
                progress_str, percent_str, pages_str, pages_left_str = burrow_util.formatProgressText(status, bookinfo, pages,
            ...
            local fontname_title = burrow_util.title_serif
            local fontname_authors = burrow_util.good_serif
            local fontname_tags = burrow_util.good_serif_it
            ...
            if self.do_filename_only or bookinfo.ignore_meta then
                ...
                title = BD.auto(title)
                fontname_title = burrow_util.good_sans
                authors = nil
            else
                title = bookinfo.title and bookinfo.title or filename_without_suffix
                title = BD.auto(title)
                local authors_limit = burrow_util.list_defaults.authors_limit_default
                if (show_series and series_mode == "series_in_separate_line") then authors_limit = burrow_util.list_defaults.authors_limit_with_series end
                authors = burrow_util.formatAuthors(bookinfo.authors, authors_limit)
            end
            ...
            if show_series then
                series = burrow_util.formatSeries(bookinfo.series, bookinfo.series_index)
            ...
            author_series = burrow_util.formatAuthorSeries(authors, series, series_mode, show_tags)

            -- tags
            if show_tags then
                tags = burrow_util.formatTags(bookinfo.keywords, burrow_util.list_defaults.tags_limit)
            else
                tags = nil
            end
            ...
            local build_wtitle = function()
                ...
                if bookinfo.unsupported or bookinfo._no_provider then
                    fontname_title = burrow_util.good_serif
                    bold_title = true
                end
            ...
            local build_wmetadata = function(width)
                ...
                if show_tags and tags then
                    fontsize_tags = math.max(burrow_util.list_defaults.tags_font_min, fontsize_authors - burrow_util.list_defaults.tags_font_offset)
            ...
            while true do
                ...
                while wtitle:isTruncated() do
                    if fontsize_title <= burrow_util.list_defaults.title_font_nominal then
                        break
                    end
            ...
            if title_reserved_space * 2 < avail_dimen_h - math.max(wmetadata:getSize().h, wright_height) then
                ...
                if title_reserved_space + math.max(wmetadata:getSize().h, wright_height) > avail_dimen_h then
                    ...
                    while title_reserved_space + math.max(wmetadata:getSize().h, wright_height) < avail_dimen_h do
                        if fontsize_title >= burrow_util.list_defaults.title_font_max then
                            break
                        end
                        fontsize_title = fontsize_title + fontsize_dec_step
                        build_multiline_wtitle()
            ...
            if dimen.h - title_reserved_space <= wmetadata:getSize().h + wright_height then
                ...
                while wmetadata:getSize().h > avail_dimen_h - title_reserved_space do
                    if fontsize_authors <= burrow_util.list_defaults.authors_font_min then
                        break
                    end
                    fontsize_authors = fontsize_authors - fontsize_dec_step
                    fontsize_title = fontsize_title - fontsize_dec_step
            ...
            if title_reserved_space + math.max(wmetadata:getSize().h, wright_height) > avail_dimen_h then
                logger.info(burrow_debug.logprefix, "Listbox height exceeded")
                logger.dbg(burrow_debug.logprefix, "dimen.h ", dimen.h)
                logger.dbg(burrow_debug.logprefix, "avail_dimen_h ", avail_dimen_h)
                logger.dbg(burrow_debug.logprefix, "title ", title)
                logger.dbg(burrow_debug.logprefix, "title_ismultiline ", title_ismultiline)
                logger.dbg(burrow_debug.logprefix, "wtitle:getSize().h ", wtitle:getSize().h)
                logger.dbg(burrow_debug.logprefix, "title_reserved_space ", title_reserved_space)
                logger.dbg(burrow_debug.logprefix, "fontsize_title ", fontsize_title)
                logger.dbg(burrow_debug.logprefix, "author_series ", author_series)
                logger.dbg(burrow_debug.logprefix, "wmetadata_iswider ", wmetadata_iswider)
                logger.dbg(burrow_debug.logprefix, "wmetadata:getSize().h ", wmetadata:getSize().h)
                logger.dbg(burrow_debug.logprefix, "wmetadata:getSize().w ", wmetadata:getSize().w)
                logger.dbg(burrow_debug.logprefix, "wmetadata_width ", wmetadata_width)
                logger.dbg(burrow_debug.logprefix, "wmetadata_reserved_space ", wmetadata_reserved_space)
                logger.dbg(burrow_debug.logprefix, "fontsize_authors ", fontsize_authors)
                logger.dbg(burrow_debug.logprefix, "wright_height ", wright_height)
                logger.dbg(burrow_debug.logprefix, "wright_width ", wright_width)
                logger.dbg(burrow_debug.logprefix, "wright_vertical_padding ", wright_vertical_padding)
            end
        ...
        elseif is_pathchooser == true then -- pathchooser mode
            ...
            local filefont = burrow_util.good_sans
            local wleft = TextBoxWidget:new {
                text = wlefttext,
            ...
            repeat
                ...
                text_widget = TextBoxWidget:new {
                    text = text .. hint,
                    face = Font:getFace(burrow_util.good_sans, fontsize_no_bookinfo),
...
function ListMenuItem:paintTo(bb, x, y)
    ...
    if x ~= math.floor(x) or y ~= math.floor(y) then
        logger.err(burrow_debug.logprefix, "ListMenuItem:paintTo() got non-integer x/y :", x, y)
    end
...
function ListMenuItem:onFocus()
    burrow_util.onFocus(self._underline_container)
    return true
end

function ListMenuItem:onUnfocus()
    burrow_util.onUnfocus(self._underline_container)
    return true
end
...
function ListMenu:_updateItemsBuildUI()
    -- Build our list
    local list_timer = burrow_debug:new()
    local line_width = self.width or self.screen_w
    table.insert(self.item_group, burrow_util.mediumBlackLine(line_width))
    local idx_offset = (self.page - 1) * self.perpage
    local select_number
    if self.recent_boundary_index == nil then self.recent_boundary_index = 0 end
    for idx = 1, self.perpage do
        local itm_timer = burrow_debug:new()
        ...
        if idx > 1 then
            ...
            if is_boundary_crossed then
                table.insert(self.item_group, burrow_util.thinGrayLine(line_width))
            else
                table.insert(self.item_group, burrow_util.thinBlackLine(line_width))
            end
        end
